...
class Minify{
		...
		if($fromCache){
			...
			foreach ($this->files as $file){
				...
				if ($mfTime>$fileTime){
	            	...
	            	$this->debug('Cache file is up to date for '.$file.' '.date('d.m.Y H:i:s', $fileTime).' > '.date('d.m.Y H:i:s', $mfTime));
	        	}
			}
		}
    ...
    private function getCacheFolder($absPath=true){
    	...
    	$cacheFolder= $this->_config->theme_abs_path . 'minify_cache/';
    	...
    	if($absPath){
    		...
    		return $this->_config->theme_web_path . 'minify_cache/';
    	}
    }
    ...
    private function minifyString($str, $processType='css'){
    	...
    	}else if ($processType=='js' && $this->minify){
