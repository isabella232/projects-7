...
class ToolsHandler extends AbstractHandler
{
	...
	public function ajaxSendFeedback() {
		$data = [];
		$data['name'] = $this->user() ? $this->user()->getFirstName() . ' ' . $this->user()
																			  ->getLastName() : $this->request('name');
	...
	public function mustacheWeby($webyId) {
		...
		$data = $this->cache()->read('weby.json.' . $webyId);
		...
		if(!$data) {
			$data = [
				'favorite_count' => $weby->getFavoriteCount(),
				'favorited_by'   => $weby->getUsersFavorited(true),
				'hits'           => $weby->getTotalHits(),
				'webyUser'       => [
					'id'        => $weby->getUser()->getId(),
					'name'      => $weby->getUser()->getUsername(),
					'avatar'    => $weby->getUser()->getAvatarUrl(),
					'followers' => $weby->getUser()->getFollowingUsersCount()
				],
				'tags'           => $weby->getTags(true),
				'is_favorited'   => $weby->inUsersFavorites(),
				'share_count'    => $weby->getShareCount()
			];

			$this->cache()->save('weby.json.' . $webyId, json_encode($data));
		} else {
		...
		$data['currentUser'] = [
			...
			'is_following' => $this->user()->isFollowing($weby->getUser())
		];
