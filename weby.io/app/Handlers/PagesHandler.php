...
class PagesHandler extends AbstractHandler
{
	...
	public function listRecentWebies($page = 1) {
		$this->helper()->logUserAction($this->user(), 'Viewing recent Webies');
		$data = WebyEntity::listRecentWebies($page, $this->_listLimit);
		$this->_listWebies($data, $page);
	}

	/**
	 * Lists popular Webies
	 *
	 * @param int $page
	 */
	public function listPopularWebies($page = 1) {
		$this->helper()->logUserAction($this->user(), 'Viewing popular Webies');
	...
	public function listWebiesByUser($username, $page = 1) {
		...
		if(!$webyUser) {
			...
			$this->helper()->logUserAction($this->user(), 'Viewing <a target="_blank" href="'.$this->app()->getConfig()->app->web_path.'user/'.$username.'">'.$username.'\'s</a> Webies');
